...
# =============================================================================
  ...
  if all_preprocessed:
    # Reload pre-processed training data from numpy dumps
    with open(data_dir + preprocessed_files[0], 'rb') as file_obj:
      print(data_dir + preprocessed_files[0])
      train_data = np.load(file_obj)
    with open(data_dir + preprocessed_files[1], 'rb') as file_obj:
      train_labels = np.load(file_obj)

    # Reload pre-processed testing data from numpy dumps
    with open(data_dir + preprocessed_files[2], 'rb') as file_obj:
      test_data = np.load(file_obj)
    with open(data_dir + preprocessed_files[3], 'rb') as file_obj:
...
def ld_cifar10(test_only=False, whitening=True):
  """
